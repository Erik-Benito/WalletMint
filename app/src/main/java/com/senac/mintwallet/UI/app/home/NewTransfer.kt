...
class NewTransfer: Fragment() {
    ...
    private fun initListeners() {
        ...
        binding.backButton.setOnClickListener {findNavController().navigate(R.id.home_graph)}
    ...
    private fun newTransfer() {
        ...
        if (amount.isEmpty() || !regex.matches(amount)) {
            return CustomToast.showToast(this.requireContext(), getString(R.string.message_amount_required), false)
        }
        ...
        transferEntity?.createdAt = LocalDateTime.now()

        val transferCreated = this.repository.createNewTransfer(transferEntity!!)

        transferEntity!!.uuid = transferCreated.uuid
        val action = NewTransferDirections.actionNewTransfer2ToRegisterNewTransfer2(transferEntity!!)
