...
class Register: Fragment() {
    ...
    private fun initListeners() {
        ...
        binding.submitButton.setOnClickListener {
            ...
            if(!validateForm(name, email, password, confirmPassword)) {
                toast.showToast(this.requireContext(), "Dados não conferem",false)
    ...
    private fun validateForm(name: String, email: String, password: String, confirmPassword: String): Boolean{
        if (name.isEmpty() || email.isEmpty() || password.isEmpty() || confirmPassword.isEmpty()) {
            toast.showToast(this.requireContext(), "Por favor, preencha todos os campos.",false)
            return false
        }
        if (password != confirmPassword) {
            toast.showToast(this.requireContext(), "As senhas não coincidem.",false)
            return false
        }
        return true
    }
    ...
    private fun createRepoUser(name: String, email: String) {
        ...
        user?.sendEmailVerification()?.addOnCompleteListener {task ->
            if (task.isSuccessful) {
                val view: View = layoutInflater.inflate(R.layout.dialog_send_email, null)
        ...
        auth
         ...
         .addOnCompleteListener(requireActivity()) {task ->
            if(!task.isSuccessful) {
                ...
                toast.showToast(this.requireContext(), message,false)
